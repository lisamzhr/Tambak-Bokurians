...
# Ubah bagian ini agar membaca dari config (.env) secara dinamis & aman
PROFILES_DIR = settings.profiles_dir
if not os.path.isabs(PROFILES_DIR):
    # Jika path-nya relatif, jadikan absolut terhadap posisi terminal saat menjalankan server
    PROFILES_DIR = os.path.abspath(os.path.join(os.getcwd(), PROFILES_DIR))


def list_profiles() -> list:
    if not os.path.isdir(PROFILES_DIR):
        print(f"⚠️ Warning: Folder profiles tidak ditemukan di: {PROFILES_DIR}")
        return []
...
def _build_generic_profile(csv_path: str) -> dict:
    ...
    return {
        ...
        "notes": "Threshold relatif ke kebiasaan kolam itu sendiri (anomaly-detection).",
